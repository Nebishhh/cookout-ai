...
- **Architectural Status**: all four planned layers are implemented and tested:
  ...
  - Persisted HTTP REST API (`@cookout-ai/api`) backed by Express and Prisma SQLite, including recipe CRUD (with step-by-step instructions), persisted "recompute live" event planning, persisted checkable shopping lists (standalone or event-linked, grouped by grocery category), Google Gemini-powered recipe import (ingredients + instructions), and real per-user email/password authentication with cookie sessions — every table is now scoped to its owning user (see §10's Authentication & Multi-Tenancy note).
  - Accessible, responsive React Web UI (`@cookout-ai/web`) built with shadcn/ui primitives and TanStack Query state management, gated behind a login/signup screen (`AuthPage.tsx`), with three top-level views (Recipes, Shopping List Builder, Event Planner) plus a four-mode AI import flow (text / URL / image upload / camera capture) inside the recipe form. The Shopping List and Event Planner views each run in two modes — build/plan a fresh one, or select a saved one from a sibling list component to view/edit/check off/delete. Every consolidated shopping list (preview or saved) renders grouped under grocery-aisle category headers (Produce, Meat, Dairy, etc.).
  - Full automated testing suite: Vitest unit/integration tests across all three workspaces + a Playwright E2E suite (recipe lifecycle, AI import fixture interception, event planning) running against real servers and an isolated database (`prisma/e2e.db`), with every spec now running as a seeded, cookie-authenticated test user (see §9).
- **Test counts** (see §9 for the full breakdown): 443 Vitest tests, 11 Playwright E2E tests, all passing.
- **Migration complete**: the legacy shared-password `accessGate` stopgap has been removed. Real per-user auth is fully built, tested, and load-bearing — every route requires a session and is scoped by `userId` — and it's the _only_ access-control layer now, verified via the full test suite plus a manual two-user pass (see §10).
...
| **Password hashing**             | bcryptjs                                                                          | latest     | Pure-JS bcrypt for signup/login credential hashing — no native module build step.                                                                                             |
| **Session cookies**              | cookie-parser                                                                     | latest     | Reads the `cookout_session` cookie on incoming requests; sessions themselves are DB-backed (SHA-256-hashed tokens), not signed/stateless.                                     |
...
│   │   │   ├── __testHelpers__/testAuth.ts  # createAuthenticatedAgent() for tests; excluded from the prod tsc build like __fixtures__
│   │   │   ├── middleware/errorHandler.ts, requireAuth.ts (session -> req.user gate), csrfGuard.ts (Origin/Referer check)
│   │   │   ├── app.ts              # Express app setup, CORS, JSON parsing, all route definitions
│   │   │   ├── auth.ts             # Password hashing, session create/lookup/delete (SHA-256-hashed tokens), signup/login
│   │   │   ├── allowedOrigins.ts   # ALLOWED_ORIGINS allowlist shared by CORS config and csrfGuard
...
│   │   ├── categoryOverrides.ts  # Per-user manual grocery-category corrections (IngredientCategoryOverride table), applied at the JSON-serialization boundary
...
│       ├── e2e/                    # Playwright specs: recipe-lifecycle, ai-import, event-planner, auth-isolation; globalSetup.ts signs up + saves session storageState
│       ├── src/
│       │   ├── components/
│       │   │   ├── ui/             # shadcn UI primitives (Button, Card, Input, etc.)
│       │   │   ├── AuthPage.tsx    # Login/signup screen, rendered by App.tsx when logged out
│       │   │   ├── Navigation.tsx  # Tab switcher: Recipes / Shopping List / Event Planner, + logged-in-as-email/logout control
...
│       │   ├── lib/                # api.ts (HTTP client), queries.ts (TanStack Query hooks), AuthContext.tsx (AuthProvider/useAuth, GET /api/auth/me on mount), formatQuantity.ts, groceryCategories.ts (category display order + grouping helper), utils.ts
│       │   └── App.tsx             # AuthProvider-wrapped; renders <AuthPage/> when logged out (see AuthContext.tsx) or the tab tree when logged in. Lifts selectedEventId/selectedShoppingListId (nullable-selected-entity pattern, same as editingRecipe) + cross-tab handoff from an event to its linked shopping list; also owns the localStorage-backed query-client persister (offline checkbox-toggle survival — see §15)
│       ├── playwright.config.ts    # Isolated ports 3010/3011, e2e.db, ALLOWED_ORIGINS=http://localhost:3010, storageState from globalSetup.ts
│       └── package.json
├── docs/
...
├── prisma/
│   ├── schema.prisma                # User, Session, Recipe, IngredientLine, RecipeStep, Event, ShoppingList, ShoppingListItem, IngredientCategoryOverride, PantryItem models
│   ├── dev.db / test.db / e2e.db    # gitignored SQLite files
├── scripts/
│   ├── smokeTestLiveGemini.js       # Manual live-API schema-drift check (documented in README, not in CI)
│   └── backfillOwnerUser.js         # One-time multi-tenancy migration step — see §10's Authentication & Multi-Tenancy note
...
- **Total Vitest Tests**: **443 passing tests** across test files spanning `packages/domain` (units incl. `Quantity.subtract()`, recipes + recipeStep incl. notes validation/trimming + stepDuration + stepTemperature, shopping + shoppingListLine + shoppingList + groceryCategory + subtractPantryStock + `practicalRounding` (Practical Scaling v2 — see §5 item 5), events + event + `parseGuestGroupText()`'s regex-extraction decision table), `apps/api` (health, recipe CRUD incl. steps with duration/temperature/notes round-trip, recipe list pagination, shopping-list preview, event-plan preview, Event CRUD, ShoppingList CRUD, event-linked ShoppingList, ingredient category overrides incl. `categoryIsOverridden` on ephemeral previews, pantry inventory subtraction, practical-scaling rounding across preview/save/event-plan routes, AI import text/URL/image incl. structured instructions with duration/temperature/notes, the shape-guard/bounded-retry `hasMinimalRecipeShape()`/`extractRecipeCandidate()` pair covering issue #1's retry-succeeds/retry-fails/no-retry-on-NoRecipeFound/no-retry-on-invalid-JSON decision table, a clean-429-message test per AI-call route (`isGeminiRateLimitError()`, see §10's AI Import Pipeline note), `POST /api/events/parse-description`'s heuristic/AI-fallback/invariant-violation/config-guard decision table, an **Authentication** block covering signup/login/logout/`me`, no-session-401, and CSRF-403 (missing-Origin and untrusted-Origin) paths, a **Guest mode** block covering guest creation, a guest agent immediately hitting a protected route, lazy-expiry-cascades-delete-the-User-and-their-data, ending a guest session via logout doing the same, and a real user's logout regression guard (only clears the Session, never deletes the User), a **Cross-user data isolation** block asserting a second user's list/get/update/delete/patch against a first user's known Recipe/Event/ShoppingList/PantryItem/IngredientCategoryOverride ids all 404 or omit the row, never leak it, `authRateLimit.test.ts` covering the login/signup/guest rate limiters' 429/header behavior in isolation plus their default no-op-under-`NODE_ENV=test` contract (see §10's rate-limiting note), `aiImportRateLimit.test.ts` covering the same for the per-user `recipeImportRateLimit` shared across import-text/url/image, plus a case confirming the bucket is keyed per-user rather than per-connection, and `guestCleanup.test.ts` unit-testing `sweepExpiredGuests()` directly (expired guest deleted with owned data, future-expiry guest untouched, a real non-guest user never touched)), and `apps/web` (App — incl. recipe steps editor + reorder + timing fields + notes field + offline-toggle retry, offline-toggle **persistence across a simulated page reload** via the localStorage-backed query-client persister, server-side search/dietary-tag filtering, event save/view/update/delete lifecycle, shopping-list save/view/toggle lifecycle, category-override editing in both build-mode and event-plan previews, a **Guest mode UI** case clicking "Continue as guest" through to a guest-labeled `Navigation` — formatQuantity, formatStepTiming). Note: the recipe-step _drag gesture_ itself has no automated coverage (jsdom stubs the layout measurement framer-motion's `Reorder` needs) — the underlying reorder state logic is covered via the pre-existing up/down-button test, which still exercises `handleMoveStep` unchanged.
- **Playwright E2E Tests**: **11 passing tests** across 5 spec files — `recipe-lifecycle.spec.ts` (full CRUD + shopping list math), `ai-import.spec.ts` (fixture-intercepted text/URL/image/camera import + a failure-path case), `event-planner.spec.ts` (happy path + 400 validation + the guest-description quick-fill's fixture-intercepted AI-fallback path), `auth-isolation.spec.ts` (signs up two real users in one browser context and asserts the second never inherits the first's cached recipes — the regression test for a real client-side cache leak found during this feature's manual verification pass, see §10), and `offline-resume.spec.ts` (pending mutation → reload while offline → transition online → automatic resume → server state updated and the persisted mutation cleared — the regression test for a real offline-resume gap found during a full production-stack acceptance test, see §10). `apps/web/e2e/globalSetup.ts` signs up one seeded e2e user directly against the API before any spec runs and saves the session cookie as Playwright `storageState` (referenced from `playwright.config.ts`'s `use.storageState`), so every spec starts already authenticated by default; `auth-isolation.spec.ts` opts out via `test.use({ storageState: { cookies: [], origins: [] } })` since it needs to exercise the real signup/login/logout UI from a clean slate. The API webServer's `env` sets `ALLOWED_ORIGINS=http://localhost:3010` to match the e2e Vite dev server's real origin, and `use.extraHTTPHeaders` sets the same `Origin` for the handful of specs that hit the API directly via Playwright's `request` fixture (which, unlike the browser, sets no `Origin` header on its own).
- **Vitest cross-file isolation**: `apps/api`'s test files share one physical SQLite file (`prisma/test.db`) via the root `DATABASE_URL`. The root `vitest.config.ts` sets `fileParallelism: false` so all test files run serially rather than racing (a per-project setting in `apps/api/vitest.config.ts` alone is not honored by the root `projects` orchestrator); every file that mutates `Recipe`/`IngredientLine` also resets those tables in a `beforeEach` as defense in depth. `apps/api/src/__testHelpers__/testAuth.ts` (excluded from the production `tsc -b` build, same as `__fixtures__/`) exports `createAuthenticatedAgent()` — signs up a fresh, uniquely-emailed user and returns a `supertest.agent(app)`-backed `TestAgent` with the session cookie and a pre-attached `Origin` header, used by every route-testing file's `beforeEach` in place of a bare `request(app)`.
...
| :------- | :------------------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `GET`    | `/api/health`                               | Health check (exempt from auth)                                                                                                                                                                         |
| `POST`   | `/api/auth/signup`                          | Create an account (email/password), log in immediately (`Set-Cookie`) — exempt from `requireAuth`, still passes through `csrfGuard`                                                                     |
| `POST`   | `/api/auth/login`                           | Log in an existing account (`Set-Cookie`) — exempt from `requireAuth`, still passes through `csrfGuard`                                                                                                 |
| `POST`   | `/api/auth/guest`                           | Zero-friction guest session — no email/password, immediately usable, data auto-deleted after ~24h or on logout (`Set-Cookie`) — exempt from `requireAuth`, still passes through `csrfGuard`             |
| `POST`   | `/api/auth/logout`                          | Delete the current session and clear the cookie — idempotent; for a guest, deletes their `User` row outright (cascade), ending the session **and** their data in one step                               |
| `GET`    | `/api/auth/me`                              | Return the current session's user — how the frontend discovers "am I logged in" on load                                                                                                                 |
...
| `PUT`    | `/api/ingredient-categories/:ingredientId`  | Set (or replace) a manual grocery-category correction for an ingredient, per-user by `ingredientId`                                                                                                     |
...
| `GET`    | `/api/pantry`                               | List all pantry items (on-hand stock, per-user by `ingredientId`)                                                                                                                                       |
...
Real per-user accounts (email + password, `bcryptjs`-hashed) replace the earlier single-shared-dataset model. `apps/api/src/auth.ts` owns credential hashing/verification and session lifecycle; `apps/api/src/middleware/requireAuth.ts` reads the `cookout_session` cookie, resolves it, and attaches `req.user = {id, email}` — mounted via `app.use('/api', requireAuth)` in front of every route except `/api/health` and `/api/auth/*`. Sessions are DB-backed, not signed/stateless: `Session.id` stores **`SHA-256(rawToken)`**, never the raw token itself (`hashToken()` in `auth.ts`) — the raw token exists only in the `Set-Cookie` header and the browser's cookie jar, so a leaked `dev.db` file alone yields no usable session. `createSession()`/`getSessionUser()`/`deleteSession()` hash-and-look-up on every call; expired sessions are opportunistically deleted on next lookup rather than swept by a background job.

**CSRF defense** (`apps/api/src/middleware/csrfGuard.ts`): the session cookie is `SameSite=Lax`, `httpOnly`, `secure` in production — but as defense in depth on top of that, every state-changing `/api/*` request (`POST`/`PUT`/`PATCH`/`DELETE`, including `/api/auth/login` and `/api/auth/signup` themselves, since login-CSRF is a distinct real attack) must carry an `Origin` (or `Referer` fallback) header matching an explicit allowlist — requests with neither header, or a non-matching one, get `403 Forbidden`. `apps/api/src/allowedOrigins.ts`'s `getAllowedOrigins()` reads the comma-separated `ALLOWED_ORIGINS` env var (default `http://localhost:3000`, the Vite dev server's origin) and is shared by both `csrfGuard` and the CORS config (`cors({ origin: (origin, cb) => cb(null, !origin || allowlist.includes(origin)), credentials: true })`, replacing a bare `cors()`'s `Access-Control-Allow-Origin: *`, which is incompatible with credentialed cookies regardless of CSRF).

**Data scoping**: `Recipe`, `Event`, `ShoppingList`, `IngredientCategoryOverride`, and `PantryItem` all gained a required `userId` column + relation to `User` (`onDelete: Cascade`). The latter two — previously global, single-`ingredientId`-keyed facts (see their own doc-comment history in `prisma/schema.prisma`) — now carry a composite `@@unique([userId, ingredientId])` instead, so pantry stock and category corrections are private per account rather than shared across every user of a deployment. `IngredientLine`/`RecipeStep`/`ShoppingListItem` don't carry their own `userId` — ownership flows through their parent. Every list route filters by `where: { userId: req.user.id }`; every detail/update/delete route resolves via `findFirst({ where: { id, userId } })` rather than `findUnique({ where: { id } })`, so a wrong-owner id 404s exactly like a nonexistent one — no existence leakage via a different status code. Recipe lookups embedded inside shopping-list/event-plan building (scaling, consolidation) are scoped the same way; a `recipeId` belonging to another user is treated identically to a deleted one, reusing the pre-existing `droppedRecipeIds` tolerance rather than a new error path.

**Migration from the pre-auth dataset**: since this project has no `prisma/migrations/` directory (schema changes apply via `npx prisma db push`, per the Commands section above), adding the required `userId` columns to tables with real existing rows was a two-step push (add as optional → backfill → flip to required). `scripts/backfillOwnerUser.js` is the one-time script that performed this: it creates a single ordinary `User` row (no elevated role/privilege — just the owner-of-record for rows that predate multi-tenancy) from `BOOTSTRAP_OWNER_EMAIL`/`BOOTSTRAP_OWNER_PASSWORD` and reassigns every pre-existing ownerless row to it.

**Frontend**: `apps/web/src/lib/AuthContext.tsx` (`AuthProvider`/`useAuth()`) calls `GET /api/auth/me` once on mount to resolve initial auth state; `apps/web/src/components/AuthPage.tsx` is the login/signup screen. `App.tsx` branches its render right after `PersistQueryClientProvider` opens — loading spinner while resolving, `<AuthPage/>` when logged out, the existing `<Navigation/>+<main>` tree when logged in — so every TanStack Query hook in that tree simply never mounts (and never fires) before login; no `enabled:` flags were needed. `Navigation.tsx` gained a logged-in-as-email + logout control next to the theme toggle. `lib/api.ts`'s single `request()` choke point gained `credentials: 'include'`, covering all endpoints in one change.

**Real bug caught by this feature's own manual two-user verification pass, not hypothetical**: the localStorage-backed query-client persister (see §15's Checkbox Toggle entry) defaulted to persisting _every_ successful query, not just the one offline-toggle mutation it was built for. Since `AppShell` fully unmounts on logout and remounts fresh on the next login, a second user logging into the same browser rehydrated the _first_ user's cached recipes straight out of localStorage — a genuine cross-user data leak on a shared device, entirely invisible to the backend's own (correctly-scoped) cross-user isolation tests, since the leak was purely client-side cache reuse across the login/logout boundary, not an API authorization gap. Fixed by adding `shouldDehydrateQuery: () => false` to `App.tsx`'s `dehydrateOptions` (so only the one allowlisted mutation ever gets written to localStorage) plus explicit cleanup (`queryClient.clear()` + removing the persister's localStorage key) on every logout. Locked in by `apps/web/e2e/auth-isolation.spec.ts`, a dedicated regression test that signs up two real users in the same browser context and asserts the second never sees the first's recipes.

**Second real bug, found during a full production-stack acceptance test (fresh Docker+Caddy build from `origin/master`), also not hypothetical**: the offline shopping-list checkbox toggle correctly paused while offline and correctly survived a page reload via the persister, but never resumed automatically once the browser came back online _later in the same session_ — reproduced twice in a real browser with an 8-second wait, only a fresh manual click reached the server. Root cause: `PersistQueryClientProvider`'s `onSuccess` in `App.tsx` calls `queryClient.resumePausedMutations()` exactly once, right after the persisted cache restores at boot — that covers "app was closed while offline, reopened later once already online," but nothing re-called it when the _already-running_ app's connectivity changed mid-session. The existing RTL regression test for this feature didn't catch it because it calls `onlineManager.setOnline(true)` _before_ remounting `<App/>`, i.e. it only exercises "already online at boot," not "boots offline, comes online later." Fixed by reusing the exact same `resumePausedMutations()` call and the same `onlineManager` TanStack Query already tracks connectivity with (not a parallel `window.addEventListener` system): `AppShell` now also subscribes to `onlineManager` and calls it on every offline→online transition, not just at mount (see the `useEffect` beside `handleLogout` in `App.tsx`). Locked in by a new Playwright spec, `apps/web/e2e/offline-resume.spec.ts`, covering the full pending-mutation → reload-while-offline → transition-online → automatic-resume → server-updated-and-mutation-cleared cycle against a real browser; the existing `App.test.tsx` RTL test still covers the narrower "already online at boot" case it always did. That spec's own doc comment is worth reading before touching this area again — it explains two non-obvious things it had to work around: a genuine `context.setOffline()` network block also breaks the reload itself (no service worker) and breaks the session (`GET /api/auth/me` fails too), and TanStack Query's `onlineManager` does not read `navigator.onLine` at startup, only real `online`/`offline` events from the moment its first consumer subscribes — too early for a separately-issued Playwright command to reliably race, hence the `addEventListener`-patching technique used there instead of a plain synthetic event dispatch.

**Cutover complete**: the original single-shared-password HTTP Basic stopgap (`accessGate.ts`) has been deleted, along with its mount in `app.ts` and `APP_ACCESS_PASSWORD` from `DEPLOY.md`/`docker-compose.yml`. `requireAuth` + per-`userId` scoping is now the only access-control layer, in both dev and production. `DEPLOY.md` documents `ALLOWED_ORIGINS` (required to match the real deployment domain, since it drives both CORS and csrfGuard). Verified live against a real Docker build, not just locally: a fresh `docker build` + container start correctly returns `401`/`403` per the new gates and issues a `Secure` session cookie under `NODE_ENV=production`. **Upgrading an already-deployed pre-auth instance is deliberately manual, not scripted** — `userId` is a required column in the schema this version ships, so the normal container startup's `prisma db push` refuses to run against pre-existing ownerless rows (by design, per the "Known limitation: destructive schema changes" note already in `DEPLOY.md`) rather than silently corrupting them; the documented fix is the same temporarily-relax-the-schema → push → run `scripts/backfillOwnerUser.js` → restore-schema → push sequence this project's own `dev.db` went through by hand (see the "Migration from the pre-auth dataset" note above), run before the container is (re)started, not through it.

**Rate limiting on `/api/auth/login` and `/api/auth/signup`** (`apps/api/src/middleware/authRateLimit.ts`, `express-rate-limit`): per-IP, standard-headers-only (`RateLimit-*`, not the legacy `X-RateLimit-*`), returning a clean `429 {error: "TooManyRequests", message}` — login is capped at 20 attempts/15min (brute-force throttling), signup at 20 accounts/hour (registration-spam throttling), each its own independent bucket. `app.set('trust proxy', 1)` was added alongside this — required for `req.ip` (what the limiter keys on) to resolve to the real client IP rather than Caddy's single upstream IP in production, or Vite's dev-proxy IP in `npm run dev`; trusting exactly one hop (not `true`, which trusts the whole chain and is what `express-rate-limit` itself warns against) is the correct setting for this app's topology, which never has more than one reverse proxy in front of it. The limiter **deliberately no-ops under `NODE_ENV=test`** (Vitest's automatic default, not something this app sets) — the existing test suite signs up dozens of users per run through a single shared supertest agent hitting one loopback IP, so a real per-IP cap would fail the suite itself rather than an attacker; `authRateLimit.test.ts` covers the limiter's actual 429/header behavior in isolation (via an explicit `skip: () => false` override) plus the default no-op-under-test contract itself. Verified against a real running dev server (not just the isolated unit test): 20 real `401`s followed by `429` on attempt 21, with `RateLimit-Limit`/`RateLimit-Remaining`/`RateLimit-Reset` headers present and signup's independent bucket unaffected.

**Guest mode**: a zero-friction alternative to real accounts — `POST /api/auth/guest` creates a `User` with `isGuest: true`, no email/password, immediately usable (same `Set-Cookie`/`requireAuth` path as signup/login), with data auto-deleted after `GUEST_SESSION_TTL_MS` (~24h) or sooner via logout. This is cheap specifically because of the multi-tenancy design above: every domain table already has a required `userId` + `onDelete: Cascade` back to `User`, so deleting one guest `User` row reclaims 100% of their data with zero other-model changes. `email`/`passwordHash` on `User` are now nullable (SQLite/Prisma allow unlimited `NULL`s in a unique index, so nullable-unique `email` never collides across guests; real users are unaffected since `signup`/`login` still always populate both) — a pure widening change, so a single `db push` sufficed, unlike the staged `userId` rollout above. `POST /api/auth/logout` doubles as "end guest session": `auth.ts`'s `endSession()` (replacing the old `deleteSession()`) branches on `isGuest` — a real user still just clears their `Session`, a guest gets their whole `User` row deleted (cascade). A guest who returns with an already-expired cookie gets the same cascade-delete treatment lazily, inline in `getSessionUser()`. A guest who never returns is the harder case: since this codebase has no other scheduled-job infrastructure, `apps/api/src/guestCleanup.ts` adds **the only cron-equivalent in the app** — a `setInterval`-based sweep (every 30min, skipped under `NODE_ENV=test` to avoid a dangling timer racing the shared `test.db`) that deletes any guest `User` whose `guestExpiresAt` has passed, via `sweepExpiredGuests()` (exported standalone so tests call it directly rather than waiting on the interval). `guestRateLimit` (10 guest sessions/hour/IP, tighter than signup's 20/hour since guest creation needs no email uniqueness check and no password — a cheaper spam vector) sits in front of the route, same `authRateLimit.ts` factory as login/signup. Frontend: `AuthPage.tsx` gained a "Continue as guest" button below the login/signup card (a separate entry point, not folded into the form); `Navigation.tsx` shows "Guest" instead of an email and relabels the logout control "End guest session" for a guest `user.isGuest`; `App.tsx`'s `handleLogout` (cache-clear + persisted-mutation localStorage cleanup, see the cross-user cache-leak bug above) needed **no changes** — it was already fully identity-agnostic, so ending a guest session automatically gets the same cross-session cache-leak defense hardened for real-user logout.

### Ingredient Category Overrides

A `IngredientCategoryOverride` Prisma model (`@@unique([userId, ingredientId])`, `category`, `updatedAt`) stores manual corrections to `categorizeIngredient()`'s keyword heuristic, keyed per-user by `ingredientId` (see the Authentication & Multi-Tenancy note above — this was globally keyed before multi-tenancy) — not scoped to one recipe or one saved list, matching how category was already treated everywhere else as a pure function of ingredient identity for that user. `apps/api/src/categoryOverrides.ts` owns validation (`category` must be one of the `GroceryCategory` enum values, else `InvalidShoppingListError` → 400) and the read/write helpers. The domain layer (`ShoppingListLine`, `ShoppingListItem`, `consolidateShoppingList`) is untouched — it keeps producing the heuristic-only category exactly as before. Every route that emits a `category` field (`serializeEventPlan()`, `toShoppingListJSON()`, and the `POST /api/shopping-list` preview) fetches the full overrides map once per request and resolves `overrides.get(ingredientId) ?? heuristicCategory` at the JSON-serialization boundary, so a correction retroactively fixes that ingredient everywhere it appears — past saved lists included, with no migration. `categoryIsOverridden` (per item, resolved the same `overrides.has(ingredientId)` way) is included on every one of these routes — `toShoppingListJSON()`, `serializeEventPlan()`, and the `POST /api/shopping-list` preview alike — so the client can always decide whether a reset control is relevant, not just on saved lists.
...
A `PantryItem` Prisma model (`@@unique([userId, ingredientId])`, `displayName`, `amount`, `unit`, `updatedAt`) stores on-hand stock as a per-user, standing fact per ingredient (was globally keyed before multi-tenancy) — same shape and rationale as `IngredientCategoryOverride`. `apps/api/src/pantryStore.ts` owns validation (delegated to the domain `Quantity` constructor — an invalid unit/negative amount throws a `DomainError`, mapped to 400 by the existing error middleware) and the read/write helpers, managed via `GET`/`PUT`/`DELETE /api/pantry/:ingredientId`.
...
**Cost-control rate limiting**: `import-text`/`import-url`/`import-image` now share one `recipeImportRateLimit` bucket (`apps/api/src/middleware/aiImportRateLimit.ts`, 30 imports/hour) — one shared bucket across all three rather than one per route, since they're the same underlying costed action (one Gemini call) just different input modes, and a per-route limit would let someone bypass it by switching modes. Unlike `authRateLimit.ts`'s login/signup/guest limiters, which key on `req.ip` (those routes run _before_ `requireAuth`), this one keys on `req.user!.id` — every route it's mounted on is registered after `app.use('/api', requireAuth)`, so a real per-user identity is always available, and per-user keying means people behind a shared/NAT'd IP don't share a bucket. Same `createXRateLimit(...)`-factory-plus-test-mode-skip shape as `authRateLimit.ts` (`aiImportRateLimit.test.ts` mirrors `authRateLimit.test.ts`'s pattern). `parse-description`'s Gemini fallback path remains uncapped — it's a different, much cheaper call site (regex-first, AI only as a fallback), not part of this pass.
...
- **Account-management flows**: password reset, email verification, and OAuth are all still unbuilt — plain email/password signup+login only (see §10's Authentication & Multi-Tenancy note for what _is_ built: real per-user accounts, cookie sessions, and full `userId` data scoping).
...
| **No Authentication / Multi-Tenancy (resolved)**        | Low      | Real per-user accounts + `userId` scoping now exist (see §10's Authentication & Multi-Tenancy note); the pre-auth shared-password `accessGate` stopgap has been removed entirely. Remaining gap: no password reset / email verification / OAuth.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Account-management flows (password reset, email verification, OAuth) are separate, not-yet-scoped future work.                                                                                                        |
...
- **Current Security Posture**: Real per-user authentication and authorization are now in place — see §10's Authentication & Multi-Tenancy note for the full design (bcrypt-hashed passwords, SHA-256-hashed DB-backed sessions, `SameSite=Lax` + explicit Origin/Referer CSRF defense, an env-controlled CORS/CSRF origin allowlist, `userId` scoping on every table with 404-not-403 on cross-owner access, per-IP rate limiting on login/signup/guest — `guestRateLimit` at 10/hour, tighter than signup's 20/hour — and per-user rate limiting on the AI recipe-import routes — `recipeImportRateLimit` at 30/hour, shared across import-text/import-url/import-image, see §10's AI Import Pipeline note). Not yet hardened against every production concern (no account-management flows).
...
- **Risks**: `POST /api/events/parse-description`'s Gemini-fallback path is still uncapped (cheaper/rarer call site than recipe import — regex-first, AI only on fallback — so lower priority, but not yet addressed); no password reset/email verification/OAuth; `guestCleanup.ts`'s 30-minute sweep interval is the app's only cron-equivalent job — a worst case of ~30min beyond a guest's 24h TTL before an abandoned guest's data is reclaimed if they never return to trigger the lazy-expiry path.

---

# 18. Current Assessment

- **Strengths**: Outstanding domain isolation, immutable value objects throughout, clean monorepo boundaries, comprehensive automated test coverage (443 Vitest + 11 Playwright E2E, now including dedicated backend and client-side cross-user data-isolation suites and a real-browser offline-resume regression suite), strict TypeScript typing, deterministic-domain/AI-at-the-edge separation maintained even as the AI import surface grew to four input modes and now extracts instructions (with per-step duration/temperature/notes) too, and even as grocery-category grouping was added via a keyword heuristic rather than a Gemini call. Real per-user authentication (bcrypt-hashed passwords, SHA-256-hashed DB-backed cookie sessions, layered CSRF/CORS origin-allowlist defense, and full `userId` scoping across every table) replaced what had been a single-shared-dataset model — verified not just by unit tests but by a dedicated isolation suite asserting one user can never list, read, update, delete, or leak-via-status-code another user's recipes, events, shopping lists, pantry, or category overrides. Recipes, events, and shopping lists are all now genuinely persisted and revisitable — not just planning tools that reset on refresh — while still cleanly separating ephemeral preview computation from deliberate, user-triggered save actions. Live Gemini calls now enforce a structured-output schema (split via `anyOf` into distinct success/error shapes, not one flat optional-everywhere object) rather than relying on prompt text alone for JSON-shape compliance, backstopped by an app-layer shape guard with one bounded, reinforced-prompt retry — verified live, not just against the SDK's types. The shopping-list checkbox toggle tolerates transient (spotty-WiFi) mutation failures via retry, **and now survives a full page reload while genuinely offline too, including resuming automatically if connectivity returns later in the same session** (not just if the app happened to already be online when it booted — a real gap found and fixed during a production-stack acceptance test, see §10), via a localStorage-backed query-client persister and an `onlineManager` subscription, verified by both an RTL integration test (unmounts/remounts `<App/>` while already online) and a real-browser Playwright spec (`offline-resume.spec.ts`, covering the harder "still offline at reload, comes online later" case). A global standing pantry can subtract on-hand stock from any generated list, and **Practical Scaling v2 rounds fractional Count-category shopping-list quantities up to a buyable whole amount** (e.g. "1 egg" not "0.25 egg"), exposing the exact mathematical figure alongside it rather than silently discarding it. A Gemini 429/rate-limit rejection is now detected specifically (`err instanceof ApiError && err.status === 429`) and returns a clean, generic message across all four AI-call routes instead of leaking the raw upstream quota-error JSON. Recipe steps carry optional duration, temperature, and free-text notes. The manual category-override editing UI (set and reset) is no longer confined to saved shopping lists — it now works identically on ephemeral previews (build-mode, event-plan preview), which re-run their underlying mutation on a successful correction since preview data isn't a cached query. The one differentiator the competitive intelligence audit found survives scrutiny — `GuestGroup`'s eligible-servings computation — now has a natural-language quick fill in front of it, deliberately kept regex-first with AI only as a fallback rather than reaching for a live model by default. Login/signup are now rate-limited against brute-force/registration-spam, and a zero-friction **guest mode** (no email/password, self-deleting after ~24h) sits alongside real accounts as a deliberate low-friction alternative — made cheap to build by the existing multi-tenancy design, since deleting one guest `User` row already cascades to reclaim 100% of their data. The three AI recipe-import routes now share a per-user rate limit too, closing off the most costly uncapped surface (each call is a real Gemini API charge) without needing per-route limits that could be dodged by switching input modes.
- **Weaknesses**: No account-management flows (password reset, email verification, OAuth); no cost-estimation feature; RecipeList's pagination doesn't extend to ShoppingListBuilder/EventPlanner's selector views, which stay intentionally unbounded; Practical Scaling v2 covers only the shopping-list-quantity rounding half of the original idea, not the batch-size (serving-count) recommendation half.
