...
- **Test counts** (see §9 for the full breakdown): 426 Vitest tests, 11 Playwright E2E tests, all passing.
...
- **Risks**: No rate limiting on any route (including login/signup — no brute-force throttling yet); no password reset/email verification/OAuth.
...
- **Weaknesses**: No account-management flows (password reset, email verification, OAuth) and no rate limiting yet on top of the new auth system; no cost-estimation feature; RecipeList's pagination doesn't extend to ShoppingListBuilder/EventPlanner's selector views, which stay intentionally unbounded; Practical Scaling v2 covers only the shopping-list-quantity rounding half of the original idea, not the batch-size (serving-count) recommendation half.
