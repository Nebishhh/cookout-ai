...
| Natural-language event-planning wrapper                               | Medium                              | High — sharpens the one proven differentiator                                   | Medium                                                            | **NEXT**                                   |
| Offline mutation persister (`@tanstack/query-sync-storage-persister`) | Medium                              | Low-Medium                                                                      | Low — already scoped in existing tech debt                        | **NEXT**                                   |
| Practical scaling v2 (non-linear rounding)                            | Medium                              | Medium — reinforces domain rigor                                                | Medium                                                            | **NEXT**                                   |
...
- Offline mutation persister for the shopping-list checkbox toggle (already scoped as a known, small gap).
- Practical scaling v2 (non-linear rounding).
- Re-run the AI-import benchmark at meaningful volume (not 2 samples) to get a real failure-rate number before making any reliability claim about it.
...
**Believe:** the AI-import pipeline's architecture (schema-enforced, deterministic-domain-boundary) is sound.
**Now know, not just believe:** it is not currently reliable — a live, direct failure was captured during this audit, on the simplest possible input. What's still unknown: the real-world failure rate. Two failures out of two live attempts is not a statistically meaningful sample. Evidence needed: running `scripts/smokeTestLiveGemini.js` (or an equivalent) across dozens of varied real recipe inputs before claiming any specific reliability number.
