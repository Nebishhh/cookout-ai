...
| Handle Gemini 429 rate-limit responses with a clean user-facing message | Medium — previously leaked a raw upstream error blob | Low-Medium — a reliability/polish fix, not a differentiator                     | Low                                                               | **SHIPPED**                                |
...
3. **No test coverage for the AI-import failure mode just discovered.** `scripts/smokeTestLiveGemini.js` exists specifically to catch schema drift against the live API, but it isn't run in CI (by design, to avoid API costs) and evidently didn't catch this before this audit did. Once the bug above is fixed, this is the mechanism that should be exercised regularly enough to prevent silent regression — not a new tool, just more disciplined use of what already exists.
   ...
   **Update, 2026-08-17 — the "why didn't it catch this" question had a concrete answer.** The script sent its own hand-written prompt with no `responseSchema`, so it was validating a request production never makes; a clean run was never evidence about the real import path. It's now `scripts/smokeTestLiveGemini.ts` and calls `parseRecipeTextWithGemini()` directly, exercising the real system prompt and the real `anyOf`/`required` schema. It also now asserts on `instructions` — which the old key list omitted entirely, meaning it could not have detected instructions or step durations disappearing, the exact drift that would silently reduce backward cook scheduling to zero-minute estimates. Verified live: a recipe stating explicit times yields durations (with "1 hour and 15 minutes" normalized to 75 minutes), and a recipe stating none yields no invented ones.
...
**Done since this roadmap was written (all five items below have shipped):**
...
- ✅ Re-ran the AI-import benchmark at volume — see §6's updated entry. Schema-compliance held (0 failures across 17 real Gemini calls); surfaced a new, separate finding instead (free-tier rate limiting).
- ✅ Handle Gemini 429/rate-limit responses cleanly. All four AI-call routes (`import-text`, `import-url`, `import-image`, `parse-description`) now detect a 429 via `err instanceof ApiError && err.status === 429` (not string-matching) and return a clean `429 {error: 'RateLimited', message: '...'}` instead of the raw upstream JSON. Verified live: a rapid-fire burst against the real API returned the clean message on every call once rate-limited, not the quota/billing JSON blob.

**NEXT (30-60 days):** none currently queued — see §6 for the next open question.
...
**A different, newly-discovered reliability problem, not the one this line used to track:** the free-tier `GEMINI_API_KEY` this project runs on allows only 5 requests/minute, and the remaining ~18 of the ~35 calls were blocked by `429 RESOURCE_EXHAUSTED`, not any extraction defect. This is not just a short burst limit — a later 5-call confirmation batch, paced 13s apart with a 35s backoff-and-retry on each rate-limited call, still failed **all 5 calls on both the first attempt and the retry**, meaning the quota stays exhausted for extended periods once a session has made moderate volume of calls, not just momentarily. **Fixed same-day**: a rate-limited call used to return the raw upstream Gemini error JSON (quota metrics, a Google Cloud console link) straight to the user via the generic `Upstream AI service error: ...` message. All four AI-call routes now detect a 429 specifically (`err instanceof ApiError && err.status === 429`, from `@google/genai`'s own typed error, not string-matching) and return a clean `429 {error: 'RateLimited', message: 'AI import is temporarily busy...'}` instead — verified live against the real, still-rate-limited key. What's still true and unaddressed: this only cleans up the _message_, not the underlying constraint — a real deployment still needs a paid tier (or its own request throttling/queueing) before AI import can be called reliable under real usage, not just schema-compliant and honest about its own limits.
