...
- **Test counts** (see §9 for the full breakdown): 334 Vitest tests, 8 Playwright E2E tests, all passing.
...
- **Total Vitest Tests**: **334 passing tests** across test files spanning `packages/domain` (units incl. `Quantity.subtract()`, recipes + recipeStep + stepDuration + stepTemperature, shopping + shoppingListLine + shoppingList + groceryCategory + subtractPantryStock, events + event), `apps/api` (health, recipe CRUD incl. steps with duration/temperature round-trip, recipe list pagination, shopping-list preview, event-plan preview, Event CRUD, ShoppingList CRUD, event-linked ShoppingList, ingredient category overrides, pantry inventory subtraction, AI import text/URL/image incl. structured instructions with duration/temperature), and `apps/web` (App — incl. recipe steps editor + reorder + timing fields + offline-toggle retry, server-side search/dietary-tag filtering, event save/view/update/delete lifecycle, shopping-list save/view/toggle lifecycle — formatQuantity, formatStepTiming). Note: the recipe-step _drag gesture_ itself has no automated coverage (jsdom stubs the layout measurement framer-motion's `Reorder` needs) — the underlying reorder state logic is covered via the pre-existing up/down-button test, which still exercises `handleMoveStep` unchanged.
...
- **Per-step notes field**: `RecipeStep` now carries optional `duration`/`temperature` (see §10, §11) but still has no free-text notes field.
...
| **Checkbox Toggle: Session-Only Resilience (resolved for the common case)** | Low      | `useToggleShoppingListItemChecked()` now retries a failed `PATCH .../items/:itemId` (3 attempts, exponential backoff) before rolling back — covers spotty-connection fetch failures, the realistic in-store scenario. A literal offline (`navigator.onLine === false`) mutation pauses and auto-resumes via TanStack Query's built-in `networkMode`, no extra wiring needed. | Doesn't survive a full page reload while offline (no persisted mutation queue) — add `@tanstack/query-sync-storage-persister` if that proves necessary in practice.                                           |
...
- **Strengths**: Outstanding domain isolation, immutable value objects throughout, clean monorepo boundaries, comprehensive automated test coverage (334 Vitest + 8 Playwright E2E), strict TypeScript typing, deterministic-domain/AI-at-the-edge separation maintained even as the AI import surface grew to four input modes and now extracts instructions (with per-step duration/temperature) too, and even as grocery-category grouping was added via a keyword heuristic rather than a Gemini call. Recipes, events, and shopping lists are all now genuinely persisted and revisitable — not just planning tools that reset on refresh — while still cleanly separating ephemeral preview computation from deliberate, user-triggered save actions. Live Gemini calls now enforce a structured-output schema rather than relying on prompt text alone for JSON-shape compliance. The shopping-list checkbox toggle tolerates transient (spotty-WiFi) mutation failures via retry, and a global standing pantry can subtract on-hand stock from any generated list.
- **Weaknesses**: Lack of authentication; no cost-estimation feature; checkbox-toggle resilience covers transient failures but not a full page reload while genuinely offline (would need a mutation persister); the manual category-override editing UI (set and reset) lives only in the saved-shopping-list view — ephemeral previews (build-mode, event-plan preview) display overrides but can't create or clear them; RecipeList's pagination doesn't extend to ShoppingListBuilder/EventPlanner's selector views, which stay intentionally unbounded; recipe steps still have no free-text notes field.
